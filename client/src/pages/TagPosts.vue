...
export default {
  name: 'TagPosts',
  meta: {
    titleTemplate: title => `${title} - TagPosts`,
    meta: {
      description: { name: 'description', content: 'TagPosts Page' },
      keywords: { name: 'keywords', content: 'TagPosts website' }
    }
  },
  data () {
    return {
  ...
  beforeMount () {
    ...
    this.$axios.get('get_tag_posts/' + this.category_id).then(res => {
      let data = res.data
